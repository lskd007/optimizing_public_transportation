...
class Line:
    ...
    def process_message(self, message):
        ...
        if topic_name.startswith('org.chicago.cta.stations.table.'): # Set the conditional correctly to the stations Faust Table
            try:
                value = json.loads(message.value())
                self._handle_station(value)
            except Exception as e:
                logger.fatal("bad station? %s, %s", value, e)
        elif topic_name.startswith('org.chicago.cta.station.arrivals.'): # Set the conditional to the arrival topic
